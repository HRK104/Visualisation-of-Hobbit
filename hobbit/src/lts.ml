...
let op_call_trans ({a; c1; c2; tr; bound = (bound10,bound20); sigma; ocall_stack; beta; beta_graph; beta_stack} :config_pair) =
  let last_move_is_sync =
    ...
    | _ -> false
  in
  if last_move_is_sync then [] else
            ...
            (fun out_lst (index,func) ->
              ...
              then failwith "ocall: g shouldn't contain ground-type values" else
                ...
                ({a = new_a;
                  ...
                  ocall_stack =  {oc_i = index;
                                  oc_gs1 = None;
                                  oc_gs2 = Some (c02.g , s2)} :: ocall_stack;
                  beta = Some new_beta;
                  beta_graph = new_beta_graph;
                  beta_stack} :: out_lst))
            []
...
let pr_transitions ({c1; c2; tr; bound = (bound10, bound20); sigma = sigma0; beta_stack} as cfg_pair) =
         ...
         let cases_of_c2 conf rest2 =
           ...
           | (Some c21, Some label2, bound2, sigma2) -> (* challenge from RHS matched by a move *)
              ...
              {cfg_pair with c1 = None;
                             ...
                             beta_stack = new_beta_stack} :: rest2
       ...
       begin
         ...
         let cases_of_c2 conf rest2 =
           ...
           | (None, None, bound2, sigma2) -> (* RHS got stuck or became bot*)
              ...
              let new_beta_stack =
                match label1 with
                | PrCall _ -> {b1=Some {g=c11.g;s=gen_s1';l=label1}; b2=None; sigma=fst gen_sigma2'} :: beta_stack
                | PrRet _ -> beta_stack
                | _ -> failwith "new_beta_stack: not Pr label"
              in
                ...
                begin
                      ...
                      | PrConst (AbsCon (id1,typ1,name1)), PrConst (AbsCon (id2,typ2,name2)) ->
                         begin
                           ...
                           | Type.IntT,Type.IntT  ->
                              ...
                              const_eq , fun_eq , (new_nc_sigma , new_nc_dtree)
                           | t1,t2 ->
                              (** BUG: T1 =/= T2 SHOULD NOT HAPPEN, THIS CAN ONLY BE IF A1 =/= A2, OR TYPE CHECKING FAILED **)
                              (assert(t1=t2);
                  ...
                  | PrCall (AbsFun (a1, _, _, _, _), prv1) ,
                    PrCall (AbsFun (a2, _, _, _, _), prv2) ->
                     ...
                     let labels_equal,new_sigma =
                       if a1 = a2 then
                         check_congruence_sat prv1 prv2 sigma2
                       else
                         false,sigma2
                     in
                     let gen_s1,gen_s2,gen_sigma2,gen1,gen2 = generalise_on_sigma new_sigma in
                     ...
                     then
                       ...
                       begin
                         ...
                         {cfg_pair with c1 = Some {c11 with s = gen_s1'};
                                        ...
                                        bound = (bound1, bound2);
                                        sigma = gen_sigma2';
                                        ocall_stack = new_oc_stack;   (** BUG FIXED?? MISSING OCALL STACK??? CHECK... **)
                           ...
                           {cfg_pair with c1 = None;
                                          ...
                                          ocall_stack = new_oc_stack;
                                          beta_stack = new_beta_stack2} :: rest2
                       end
                  | _ ->
                     ...
                     {cfg_pair with c1 = Some {c11 with s = gen_s1'};
                                    ...
                                    bound = (bound1, bound2);
                                    sigma = gen_sigma2';
                                    ocall_stack = new_oc_stack;   (** BUG FIXED?? OCALL STACK??? CHECK... 
                                                                      maybe in BOT mode makes no difference. 
                                                                      Can't prove equivalence in BOT mode... **)
                                    beta_stack = new_beta_stack1} ::
                       {cfg_pair with c1 = None;
                                      c2 = Some {c21 with s = gen_s2'};
                                      tr = label2 :: (Markup "LHS=bot") :: tr;
...
let pr_moves cfg_pair =
  let pr_cfgs = pr_transitions (pr_upto_techniques cfg_pair) in
