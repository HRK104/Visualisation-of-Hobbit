...
let rec refresh_bound_ids_gprop_rec map g refresh_id =
  ...
  | GIdent (id,p) ->
     (match IntMap.find_opt id.idid map with
      | None -> raise (Error.RuntimeE (p, "gprop unbound variable: " ^ id.str)) (* should not happen *)
     ...
     (* TODO: GEN *)
      ...
      let newgen =
        ...
        | Some (is, loc_exp_list, gprop) ->
           let newis,newmap =
             List.fold_right
               (fun (i,p) (newis,newmap) ->
                 let newi = refresh_id i in
                 (newi,p)::newis, extend i.str i.idid newi.idid newmap) is ([],map)
           in
           let newis = List.rev newis in
