  ...
  | AbsFun (i, t1, t2, s, gen) ->
     let newgen = match gen with
       ...
       | Some (ws, ls, gprop) ->
          ...
          Some (newws, newls, newgprop)
     in
     AbsFun (i, normalise_type mode t1, normalise_type mode t2, s, newgen)
...
let rec type_of_val tenv v =
  ...
  | FunVal (f, ft, p, pt, e1, gen) ->
     (* TODO: GEN *)
      ...
      fun_t
  | AbsCon (uid, t, s) -> t
  | AbsFun (uid, t1, t2, s, gen) ->
     (** TODO: is this the correct way to add GEN????? **)
     begin
       match gen with
       | None -> ()
       | Some (idts , loc_exp_list , g_prop) ->
...
try
  ...
  | IdentExp (id, _) ->
     begin
       match lookup id.idid tenv with
       | None ->
          raise (Error.TypeE(Ast.get_lex_pos e, "type_of_exp: unbound variable: " ^ id.str))
       (* this is checked in ast.ml so it should not be raised here *)
       | Some ty -> ty
     end
  | ArithExp (op, rands, p) ->
...
and debug_get_tvars_val = function
  ...
  | AbsFun (f, ft, pt, s, gen) ->
     (* TODO: GEN *)
     (debug_get_tvars_type ft) @ (debug_get_tvars_type pt)
  | _ -> []
